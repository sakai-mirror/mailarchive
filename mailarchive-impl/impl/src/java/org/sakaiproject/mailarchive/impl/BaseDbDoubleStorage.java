...
public class BaseDbDoubleStorage
{
    ...
    /** only issue warning message about matchEntity once */
...
System.out.println("COUNT Sql I="+sql);
	...
	public List getAllResources(Entity container, String filter, String search, boolean asc, PagingPosition pager)
	{
        ...
        if ( pager != null && search == null)
        {
            ...
            if ( limitedSql != null ) 
            {
                pagedInSql = true;
                sql = limitedSql;
            // If we cannot use Limit - at least use TOP to trim the end 
            // of the Record Set
            } else {
    ...
    // This needs to be over-ridden for match to function
